...
| 3 | Burden philosophy | 25/50/100 mV and range-dependent | **Range-dependent 25 mV (10mA/1mA), 50 mV (100uA/10uA), 100 mV (1uA/100nA) — IR-05 canonical (§2.4)** | 100 mV =5% @2 V/16% @0.6 V not harmless — halving burden on low-V ranges costs 3.16× Johnson but halves DUT error | Yes — headroom vs noise |
...
| 6 | Stored-energy management | low C, damping R, isolation R, active discharge | **C_UPSTREAM ≤10 nF before R_iso; C_DOWNSTREAM ≤80-150 pF after R_iso (IR-14)** | E=0.5CV²: 10 nF@5 V=125 nJ dominates filament; 100 nF cable=1.25 µJ → limit cable length/C | Yes |
| 7 | DAC class | AD5686R (16-bit 2 LSB INL), AD5764 (±10 V ±1 LSB), AD5791 20-bit | **AD5686R (LSB 152.6µV on 10V) vs AD5764 (20V span, LSB 305.2µV, ±11.4V min, no ±5V mode — IR-06/07) — INL in volts equal, do not promote on INL alone** | AD5686R LSB 152.6µV (10V span); AD5764 20V span LSB 305.2µV, no ±5V mode, ±11.4V min supply — INL ±1 LSB = ±305µV vs AD5686R ±2 LSB = ±305µV — equal in volts | Yes — error budget |
...
| 9 | Precision amps | ADA4522 (zero-drift 55 V), OPA189/188, ADA4528, OPA140 (JFET), LTC2057 | **Role-dependent: ADA4522-2 for shunt loop, OPA140/JFET for DUT sense ≤10pA (IR-02)** | Not one amp everywhere; Ib matters for high-Z | No |
| 10 | ADC class | ADS1262 (38 kSPS), ADS124S08, AD7175 (±1 ppm INL), AD7124 | **ADS1262 vs AD7175 with bipolar front-end A/B/C per IR-12** (both PGA, 50 Hz rejection) | ADS1262 32-bit word ≠ performance; compare RMS/noise-free at 50 Hz | Yes — noise @NPLC |
...
| 12 | Kelvin loop | FORCE vs SENSE feedback, burden placement | **SENSE feedback at DUT via high-Z buffers >10GΩ, V_FORCE=V_DUT+V_SHUNT+I*R_LEAD (IR-11), switched open-sense IR-03** | Kelvin correctness: V_DUT = V_SENSE, not V_FORCE - V_burden | Yes — stability |
| 13 | Grounding | single plane partitioned, split, star, isolated | **One continuous reference plane, no etched AGND/DGND split (IR-13)** | One continuous reference plane; separation by placement/return-current control; no etched split/bridge — single tie at ADC is measurement point (IR-13) | No |
...
| 17 | Power tree | ±12 V ext → LDO domains | **Ext ±12V → Option A raw ±12 for power stage; +5 prec via LT1763 positive; neg rail LT1964-class (IR-07)** | Option A raw ±12 for power stage (AD5764 ±11.4V compatible); +5 prec via LT1763; neg via LT1964-class; ±10V LDO not AD5764-compatible (IR-07) | No |
...
| 21 | Source Candidate C | Composite outer loop + LT1970A booster | **Composite outer loop+LT1970A booster — DEC-025/IR-15 — SELECTED FOR PHASE 3** | Precision outer amp drives LT1970A as booster; retains LT1970A drive/limit/flags with precision offset | Yes — nested loop stability |
